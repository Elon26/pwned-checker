    ...
    static async readFile(path) {
        return new Promise((resolve, reject) => {
            fs.readFile(path, 'utf8', (err, data) => {
                if (err || !data) {
                    AutoFillCredentialProviderLog_ts_1.AutoFillCredentialProviderLog.error("Couldn't read file:" + path);
                    reject(err);
                    return;
                }
                resolve(data);
    ...
    static async writeFile(path, contents) {
        return new Promise((resolve, reject) => {
            fs.writeFile(path, contents, 'utf8', (err) => {
                if (err) {
                    AutoFillCredentialProviderLog_ts_1.AutoFillCredentialProviderLog.error("Couldn't write file:" + path);
                    reject(err);
                    return;
                }
                resolve();
