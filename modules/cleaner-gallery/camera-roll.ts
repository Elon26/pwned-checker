...
import type { GalleryAsset, GetAssetsParams } from './types';
import { buildResultAsset } from './utils/build-result-asset';
export const GalleryModule = CleanerGalleryModule;
/**
...
export class CameraRoll {
  ...
  static async getAssets(params: GetAssetsParams): Promise<GalleryAsset[]> {
    const { items } = await CleanerGalleryModule.getAssets(
      Platform.select({
        ios: {
          ...params,
  ...
  static async findSimilarImages(interval = 15): Promise<GalleryAsset[][]> {
    const groups = await CleanerGalleryModule.findSimilarImagesFromGallery(interval);

    return groups.map((items: any[]) => items.map((x) => buildResultAsset({ ...x, mediaType: 1 })));
  }
  ...
  static async findBlurryImages({
    ...
    const { items, processedIds } = await CleanerGalleryModule.findBlurryImagesFromGallery(
      ignoreIds,
      threshold,
      itemsPerPage
    );
  ...
  static async getAssetsCount(
    ...
    const result = await CleanerGalleryModule.getAssets({
      ...params,
      totalOnly: true,
    });
  ...
  //   return CleanerGalleryModule.editIsFavorite(id, value);
  // }
  ...
  static deleteAssets(ids: string[]): Promise<{ success: boolean }> {
    return CleanerGalleryModule.deleteAssets(ids);
  }
  ...
  //   return CleanerGalleryModule.getAssetVideoInfo(id);
  // }
  ...
  static saveAssets(files: string[]): Promise<void> {
    return CleanerGalleryModule.saveAssets(files);
  }
  ...
  static extractThumbnail(
    ...
    return CleanerGalleryModule.extractThumbnail(id, filename, width, height);
  }
