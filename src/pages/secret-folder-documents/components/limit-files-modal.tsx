...
type LimitFilesModalProps = ModalComponentProp<
  ModalStackParams,
  void,
  'LimitFilesModal'
>;

export function LimitFilesModal({
  modal: { params, closeModal },
}: LimitFilesModalProps) {
    ...
    <View className="rounded-2xl bg-background gap-4 p-4 w-72">
      <View className="items-center gap-3 py-4">
        <UiText className="text-center text-xl font-semibold">
          You’ve reached the storage limit of {params?.count ?? 0} files
        </UiText>
        <UiText className="text-center text-sm">
          Upgrade to premium for unlimited file saving in Private Space.
        </UiText>
      </View>
